...
(function () {
  ...
  function buildStrip() {
    ...
    // The session playlist is now a first-class "Session Setlist" panel in
    // dash.jsx (per-track download + delete, now-playing marker). The old
    // bottom-bar Download button and Songs slide-out were removed to avoid
    // duplicating it.
    ...
    // (The dashboard layout is now fluid CSS — it no longer scales to fit, so the
    // old resize re-dispatch hack that fed the player height into dash.jsx is gone.)
  }
