/* ============================================================
   ...
   download for saving specific songs. There is no real audio
   in the prototype — playback + downloads are simulated.
   ============================================================ */
...
function ScreenRecap({ onBack }) {
  ...
  const saveOne = (t) => { if (savedIds[t.id]) return; setSavedIds((s) => ({ ...s, [t.id]: true })); ping('Track saved ✓'); };
  const saveAll = () => { const all = {}; tracks.forEach((t) => { all[t.id] = true; }); setSavedIds(all); setAllSaved(true); ping('All tracks saved ✓'); haptic([10, 40, 10]); };
  const copyLink = () => {
    try { navigator.clipboard && navigator.clipboard.writeText('https://theshow.live/set'); } catch (e) {}
